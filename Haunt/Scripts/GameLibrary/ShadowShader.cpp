...
HRESULT ShadowShader::CreateShader()
{
	HRESULT hr = S_OK;
	...
	// 頂点シェーダの読み込み
	auto vertexShaderBlob = DX::ReadData(L"Shader\\ShadowVS.cso");
	...
	// ピクセルシェーダの読み込み
	auto pixelShaderBlob = DX::ReadData(L"Shader\\ShadowPS.cso");
	...
	blendState.RenderTarget[0].DestBlend = D3D11_BLEND_INV_SRC_COLOR;
...
void ShadowShader::SetRenderShader()
{
...
void ShadowShader::SetWorld(XMMATRIX world)
{
	m_World = world;
}

void ShadowShader::SetView(XMMATRIX view)
{
	m_View = view;
}

void ShadowShader::SetProjection(XMMATRIX projection)
{
