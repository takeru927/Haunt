...
AnimationShader::AnimationShader()
	...
	, m_CBMatrix(nullptr), m_CBMaterial(nullptr), m_CBLighting(nullptr), m_CBBones(nullptr)
	, m_World(XMMatrixIdentity()), m_View(XMMatrixIdentity()), m_Projection(XMMatrixIdentity())
{
}

HRESULT AnimationShader::CreateShader()
{
	HRESULT hr = S_OK;

	auto device = D3D->GetDevice();

	auto vertexShaderGraph = DX::ReadData(L"Shader\\AnimationVS.cso");
	...
	if (FAILED(hr))
		return hr;
	constantBuffer.ByteWidth = sizeof(CBBones);
	hr = device->CreateBuffer(&constantBuffer, nullptr, m_CBBones.ReleaseAndGetAddressOf());
	if (FAILED(hr))
		return hr;
...
void AnimationShader::SetRenderShader()
{
	...
	context->VSSetConstantBuffers(3, 1, m_CBBones.GetAddressOf());
...
void AnimationShader::SetWorld(XMMATRIX world)
{
	m_World = world;
}

void AnimationShader::SetView(XMMATRIX view)
{
	m_View = view;
}

void AnimationShader::SetProjection(XMMATRIX projection)
{
	m_Projection = projection;
}

void AnimationShader::SetDirectionalLight(XMFLOAT4 lightDir, XMFLOAT4 lightAmbient, XMFLOAT4 lightDiffuse)
{
	auto context = D3D->GetDeviceContext();
...
void AnimationShader::SetMaterial(XMFLOAT4 ambient, XMFLOAT4 diffuse, bool useTex)
{
	auto context = D3D->GetDeviceContext();
...
void AnimationShader::SetBones(const XMMATRIX bones[])
{
	auto context = D3D->GetDeviceContext();

	// 定数バッファの設定
	D3D11_MAPPED_SUBRESOURCE mappedResource = {};
	context->Map(m_CBBones.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	auto cbBones = (CBBones*)mappedResource.pData;
	memcpy_s(cbBones, sizeof(CBBones), bones, sizeof(CBBones));
	context->Unmap(m_CBBones.Get(), 0);
}